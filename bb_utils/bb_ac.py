...
class _BFSubcluster:
    ...
    def update(self, subcluster):
        #self.av_props = (self.av_props * self.n_samples_ + subcluster.av_props * subcluster.n_samples_)/(self.n_samples_ + subcluster.n_samples_)
    ...
    def merge_subcluster(self, nominee_cluster, threshold):
        ...
        if new_n == 2:
            jt_sim = pair_sim(self.linear_sum_, nominee_cluster.linear_sum_) * 2
        else:
            #jt_sim = jt_isim(new_ls + new_centroid, new_n + 1) * (new_n + 1) - jt_isim(new_ls, new_n) * (new_n - 1)
            jt_sim = jt_isim(new_ls, new_n) * 2
        ...
        if AC:
            if jt_sim >= threshold*2 and ((abs(self.p_max - nominee_cluster.p_min) > 1) or 
                                          (abs(self.p_max - nominee_cluster.p_max) > 1) or 
                                          (abs(self.p_min - nominee_cluster.p_min) > 1) or 
                                          (abs(self.p_min - nominee_cluster.p_max) > 1)):
                (
                self.n_samples_,
                self.linear_sum_,
                self.centroid_,
