...
# The new centroid vector has n_features + 1 components
# the first n_features are the linear sum of elements in the cluster, the last one is the number of elements in the cluster
...
def jt_cent(cent1, cent2):
    comb_cent = cent1 + cent2
    c_total = comb_cent[:-1]
    n_objects = comb_cent[-1]
    return jt_isim(c_total, n_objects)

def jt_cent_inter(cent1, cent2):
    c_total1 =  cent1[:-1]
    n_objects1 = cent1[-1]
    c_total2 =  cent2[:-1]
    n_objects2 = cent2[-1]
    
    return (jt_cent(cent1, cent2) * (n_objects1 + n_objects2) * (n_objects1 + n_objects2 - 1) - (jt_isim(c_total1, n_objects1) * n_objects1 * (n_objects1 - 1) + jt_isim(c_total2, n_objects2) * n_objects2 * (n_objects2 - 1)))/(2 * n_objects1 * n_objects2)
...
def max_separation(X):
    ...
    for cent1 in X:
        isim_matrix.append([])
        for cent2 in X:
            isim_matrix[-1].append(jt_cent_inter(cent1, cent2))
            #isim_matrix[-1].append(jt_cent(cent1, cent2))
    isim_matrix = np.array(isim_matrix)
    #np.fill_diagonal(isim_matrix, 0)
    (mol1, mol2) = np.unravel_index(np.argmin(isim_matrix), isim_matrix.shape)
    sims_mol1 = isim_matrix[mol1]
    #sims_mol1[mol1] = 3.08
    sims_mol2 = isim_matrix[mol2]
    #sims_mol2[mol2] = 3.08
...
class _BFNode:
    ...
    def __init__(self, *, threshold, branching_factor, is_leaf, n_features, dtype):
        ...
        self.subclusters_ = []
        self.init_centroids_ = np.zeros((branching_factor + 1, n_features + 1), dtype=dtype)
    ...
    def insert_bf_subcluster(self, subcluster, set_bits):
        ...
        sim_matrix = []
        for cent in self.centroids_:
            sim_matrix.append(jt_cent_inter(cent, subcluster.centroid_))
            #sim_matrix.append(jt_cent(cent, subcluster.centroid_))
...
class _BFSubcluster:
    ...
    def __init__(self, *, linear_sum = None, mol_indices = [], p_min = 0, p_max = 0):
        if linear_sum is None:
            ...
            self.centroid_ = np.append(self.linear_sum_, 1)
    ...
    def update(self, subcluster):
        ...
        self.centroid_ += subcluster.centroid_
...
#
#def count_pairs(fps, props, thre, k):
#    mol_inds = []
#    comps = []
...
#                pass
#            else:
#                if pair_sim(m1, m2) >= thre and abs(props[i1] - props[i2]) >= 1:
#                    comps.append(1)
...
#fps = np.load('fps.npy')
#
#def close_analysis(fps, props, thre=0.95):
#    #thre = 0.95
#    brc = BitBirch(branching_factor = 50, threshold = thre)
#    brc.fit(fps, props)
#    
...
#            #tot += len(k) * (len(k) - 1)/2
#            #print(k)
#            n_pairs, new_inds = count_pairs(fps[k], props[k], thre, k)
#            tot += n_pairs
...
#while tot:
#    print(r)
#    tot, total_indices = close_analysis(new_fps, new_props, thre=0.9)
#    print(tot)
#    total_pairs += tot
...
##                pass
##            else:
##                if pair_sim(m1, m2) >= 0.9:
##                    comps.append(1)
##                    #print(i1, i2)
##    comps = np.array(comps)
##    return np.sum(comps)/2
##
##fps = np.load('fps.npy')
##props=np.load('props.npy')
##
##def count_acs(fps, props, thre):
##    sim_matrix=[]
##    prop_matrix = []
...
##            total += count_acs(fps[k], props[k], thre)
##            # 'unique' acs
##            unique += 1
