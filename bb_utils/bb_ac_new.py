...
#globals()['activity_cliffs'] = activity_cliffs

activity_cliffs = True
...
def max_separation(X):
    ...
    dot_to_cent = np.dot(X, centroid)
    mol1 = np.argmin(dot_to_cent)
    sims_mol1 = np.dot(X, X[mol1])
    mol2 = np.argmin(sims_mol1)
    sims_mol2 = np.dot(X, X[mol2])
...
class _BFSubcluster:
    ...
    def merge_subcluster(self, nominee_cluster, threshold):
        ...
        if activity_cliffs:
            ...
            if jt_sim >= threshold and new_max - new_min < 1:
                (
                    self.n_samples_,
                    self.linear_sum_,
                    self.centroid_,
